...
// Type for native functions registered with Lua.
pub type lua_CFunction = Option<extern "C" fn(L: *mut lua_State) -> c_int>;

// Type for continuation functions.
pub type lua_KFunction = Option<extern "C" fn(L: *mut lua_State, status: c_int, ctx: lua_KContext)
                                              -> c_int>;

// Type for functions that read/write blocks when loading/dumping Lua chunks.
pub type lua_Reader = Option<extern "C" fn(L: *mut lua_State, ud: *mut c_void, sz: *mut size_t)
                                           -> *const c_char>;

pub type lua_Writer = Option<extern "C" fn(L: *mut lua_State,
                                           p: *const c_void,
                                           sz: size_t,
                                           ud: *mut c_void)
                                           -> c_int>;
...
pub type lua_Alloc = Option<extern "C" fn(ud: *mut c_void,
                                          ptr: *mut c_void,
                                          osize: size_t,
                                          nsize: size_t)
                                          -> *mut c_void>;
...
pub const LUA_OPLT: c_int = 0;
pub const LUA_OPLE: c_int = 0;
...
// Useful macros.
// Implemented as inlined rust functions.

#[inline(always)]
pub unsafe fn lua_getextraspace(L: *mut lua_State) -> *mut c_void {
    L.offset(-luaconf::LUA_EXTRASPACE as isize) as *mut c_void
}
...
pub unsafe fn lua_pushliteral(L: *mut lua_State, s: &'static str) -> *const c_char {
    ...
    lua_pushlstring(L, p, s.len() as size_t)
}
...
pub const LUA_MASKRET: c_int = 2 << (LUA_HOOKRET as usize);
pub const LUA_MASKLINE: c_int = 3 << (LUA_HOOKLINE as usize);
pub const LUA_MASKCOUNT: c_int = 4 << (LUA_HOOKCOUNT as usize);
...
pub struct lua_Debug {
    ...
    pub short_source: [c_char; luaconf::LUA_IDSIZE as usize],
    i_ci: *mut c_void,
}
